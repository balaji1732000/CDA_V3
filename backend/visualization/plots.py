...
def confusion_metrix(model_name, model, X_test, Y_test):
    ...
    sns_heatmap = sns.heatmap(
        matrix, annot=True, cmap="Blues", fmt="g", annot_kws={"size": 20}
    )
...
def plot_clusters(X, labels):
    ...
    for color, label in zip(colors, unique_labels):
        idx = labels == label
        ax.scatter(
            X_pca[idx, 0], X_pca[idx, 1], color=color, label=f"Cluster {label}", s=50
        )
...
def plot_qq_plot(y_pred, Y_test):
    ...
    ax.scatter(
        osm, osr, alpha=0.8, edgecolors="#e8b517", c="yellow", label="Data Points"
    )
